...
what is `struct in_addr sin_addr`?
...
> Imagine your computer is a large apartment building with multiple entrances - network interfaces (ethernet, WiFi). **Sockets are like individual mailboxes inside the building, designated locations for receiving messages** (data). Binding a socket to a local address is like **assigning a specific mailbox to an apartment** (door number = port, floor = IP address). This tells the mail carrier (network) where to deliver incoming mail (data packets) for that particular resident (application).
